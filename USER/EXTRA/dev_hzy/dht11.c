...
// 按需更新温湿度数据
void DHT11_Update_Data_If_Expired(void) {
    if (tick_expired(&last_update_tick, 1000)) { // 每秒更新一次数据
        u8 temp, humi;
...
// 获取温度
u8 DHT11_Get_Temperature(void) {
    return temperature;
}

// 获取湿度
u8 DHT11_Get_Humidity(void) {
    return humidity;
}